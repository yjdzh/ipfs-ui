		...
		<div slot="table">
			...
			<Modal v-model="modal1" width="500" @on-cancel="cancel1">
				...
				<div>
					...
					<br />
					<Input v-model="value1" type="textarea" :autosize="{minRows: 5,maxRows: 8}" placeholder="可以输入要发货设备的mac，并使用','或者'，'进行分割,如:00E0701355D1，00E0701355D2"></Input>
				</div>
				<div slot="footer">
			...
			<Modal v-model="modal2" width="700" @on-cancel="cancel2">
				...
				<div>
					...
					<Scroll :on-reach-bottom="handleReachBottom2" v-if="zoneId">
						<evtab border :columns="datahead2" :data="databody2" size='small' :loading="loading2" @on-select="select2"
						 @on-select-cancel="select2" @on-select-all-cancel="selectall2" @on-select-all="selectall2">
						</evtab>
					</Scroll>
				</div>
				<!--<div>-->
			...
			<Modal v-model="modal3" width="700" @on-cancel="cancel3">
				...
				<div>
					<Scroll :on-reach-bottom="handleReachBottom3">
						<evtab border :columns="datahead3" :data="databody3" size='small' :loading="loading3" @on-select="select3"
						 @on-select-cancel="select3" @on-select-all-cancel="selectall3" @on-select-all="selectall3">
						</evtab>
					</Scroll>
				</div>
				<!--<div>-->
